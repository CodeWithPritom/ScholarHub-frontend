# Tab-Switch Refresh Fix — Implementation Walkthrough

## Root Cause

The app was **not** using explicit `visibilitychange` or `window.onfocus` listeners. The actual culprit was **Supabase's internal behavior**:
...
sequenceDiagram
    ...
    User->>Browser: Switch back
    Browser->>Supabase SDK: visibilitychange → "visible"
    Note over Supabase SDK: Internal token refresh logic
    Supabase SDK->>App.jsx: onAuthStateChange("TOKEN_REFRESHED")
    App.jsx->>App.jsx: setIsInitializing(true) 💥
    Note over App.jsx: Full-screen loading spinner renders
    Note over AdminPanel: UNMOUNTED — all state lost
    App.jsx->>App.jsx: fetchAndSetProfile() → setIsInitializing(false)
    Note over AdminPanel: RE-MOUNTED — fetches from scratch
```

**Line 149 was the bomb:** `if (isMounted) setIsInitializing(true);` — this ran for **every** auth event, including `TOKEN_REFRESHED` which Supabase fires on every tab re-focus.

## Files Changed

### 1. [App.jsx](file:///e:/Websites/NCBI/frontend/src/App.jsx) — Event Filtering
...
**Key changes:**
- `onAuthStateChange` now classifies events as **significant** vs **silent**
- Only `SIGNED_IN`, `SIGNED_OUT`, `INITIAL_SESSION`, `USER_UPDATED`, `PASSWORD_RECOVERY` show the loading spinner
- `TOKEN_REFRESHED` silently updates the `user` object without flashing UI or unmounting children
- The 5-minute background interval now does a **silent** profile refresh (no `setIsInitializing(true)`)

### 2. [AdminPanel.jsx](file:///e:/Websites/NCBI/frontend/src/pages/AdminPanel.jsx) — Persistent State
...
**Key changes:**
- Added `loadedTabs` Set to track which tabs have already fetched their data
- `useEffect` manager now skips fetching if the tab is already loaded
- `users` and `expiry` tabs share `usersList`, so loading one marks both as loaded
- Added a **manual Refresh button** next to the tab bar — admin explicitly clicks it when they want fresh data
- `handleManualRefresh()` clears the current tab's "loaded" flag, which triggers the effect to re-fetch

### 3. [ResearchPage.jsx](file:///e:/Websites/NCBI/frontend/src/ResearchPage.jsx) — No Changes Needed

> [!NOTE]
> ResearchPage was **already clean**. It has no `visibilitychange` or `onfocus` listeners. Its `AbortController.abort()` calls are exclusively triggered by:
> - **User initiating a new search** (line 1019–1020) — correctly cancels previous request
> - **User clicking "Cancel"** (line 944–950) — manual cancellation
> - **Timeout safety nets** (90s/60s timeouts on AI calls)
>
...
|----------|--------|-------|
| Switch tabs and back | Full loading spinner → data re-fetched | **Nothing happens** — UI stays exactly where you left it |
| Supabase token refresh | `setIsInitializing(true)` → remount all | Silent `setUser()` update, children unaffected |
| 5-min background refresh | Full profile re-fetch with loading flash | Silent profile update, no visual change |
| Admin switches tabs | Previous tab data lost | Data persists in state, no re-fetch |
| Admin wants fresh data | Switch away and back | Click **Refresh** button explicitly |
| AI fetch in progress during tab switch | Continues (no abort) | Continues (no abort) — unchanged |

## Build Status

✅ `vite build` — **0 errors**, built in 354ms.
